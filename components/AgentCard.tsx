...
export default function AgentCard({ agent }: { agent: RegistryAgent }) {
  ...
  const style =
    statusStyles[agent.status as keyof typeof statusStyles] ?? statusStyles.unknown;

  return (
    <article className="group relative overflow-hidden rounded-xl border border-white/10 bg-zinc-950/60 p5 backdrop-blur transition-all hover:border-white/20">
