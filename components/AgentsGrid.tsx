...
const FALLBACK_REGISTRY: RegistryAgent[] = [
  ...
  },
  {
    id: 'axiom',
    name: 'Axiom',
    headline: 'Reasoning and validation agent — keeps the system correct and consistent.',
    status: 'online',
  },
  {
    id: 'aether',
    name: 'Aether',
    headline: 'Orchestration and monitoring agent — routes tasks and watches telemetry.',
    status: 'degraded',
  },
]

const STATUS_SET = new Set<RegistryAgent['status']>(['online', 'offline', 'degraded', 'unknown'])
...
function coerceAgent(input: unknown): RegistryAgent | null {
  ...
  const status = STATUS_SET.has(statusRaw as any) ? (statusRaw as any) : 'unknown'
  if (!id || !name || !headline) return null
  return { id, name, headline, status }
}

function extractAgents(payload: unknown): RegistryAgent[] {
  const candidates: unknow[] = Array.isArray(payload)
    ? payload
    : payload && typeof payload === 'object' && Array.isArray((payload as any).agents)
      ? ((payload as any).agents as unknow[])
      : []
  return candidates.map(coerceAgent).filter((a): a is RegistryAgent => Boolean(a))
}
  ...
  return (
    <section className="w-full">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold tracking-wider text-white/90">Agents</h2>
